...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		if (platform === 'win32') {
			...
			const contextMenu = (product as { win32ContextMenu?: Record<string, { clsid: string }> }).win32ContextMenu?.[arch];
			if ((quality === 'stable' || quality === 'insider') && contextMenu) {
				...
				result = es.merge(result, gulp.src('resources/win32/appx/AppxManifest.xml', { base: '.' })
					...
					.pipe(replace('@@FileExplorerContextMenuCLSID@@', contextMenu.clsid))
